...
def judge_smoke(pose_result,img,cgr):
    """
    判断是否出现吸烟行为。
    ...
    left_angle, right_angle = cal_angle(k.data[0])
    left_hand_index = 9
    right_hand_index = 10

    if int(left_angle) < 50:
        dis = cal_dis(k.xy[0], left_hand_index)
        if dis < 0.8 and cgr_detect(pose_result, img, cgr, left_hand_index):
            return 2
        else:
            return 1

    elif int(right_angle) < 50:
        dis = cal_dis(k.xy[0], right_hand_index)
        if dis < 0.8 and cgr_detect(pose_result, img, cgr, right_hand_index):
            return 2
        else:
            return 1
...
def detect_and_draw(pose_result, img,cgr):
    # lw = line_width or max(round(sum(img.shape) / 2 * 0.003), 2) # 线宽

    # 画人物框
    for d in pose_result:
        c, conf, id = int(d.boxes.cls), float(d.boxes.conf), None if d.boxes.id is None else int(d.boxes.id.item())
        if id not in ids:
            ids[id]=np.array([id,0,0,False])
        # name = ('' if id is None else f'id:{id} ') + pose_result.names[c]
        # label = (f'{name} {conf:.2f}' if conf else name)
        # if conf > 0.3:
        #     box_label(d.boxes.xyxy.squeeze(), img, lw, label)

        # 时长判断
        status=judge_smoke(d, img, cgr)
        if status==2:
            if ids[id][1]<smoking_threshold:
                box_label(d.boxes.xyxy.squeeze(), img, 3, "suspicious", (0, 255, 255))
            ids[id][1] += 1
            if ids[id][2]>0:
                ids[id][2] -= 1
        elif status==1:
        ...
        if ids[id][1] >= smoking_threshold:
            ids[id][3]= True
            # print(f"{id} 吸烟")
            box_label(d.boxes.xyxy.squeeze(), img, 3,"id"+str(id)+"smoking", (0, 0, 255))

        elif ids[id][2] >= no_smoking_threshold:
            ids[id][3] = False
            # print(f"{id} 没有吸烟")
    ...
    # 画骨架
    # for k in pose_result.keypoints.data:
    #     key_label(k, img,img.shape, kpt_line=True)
...
def cgr_detect(k, img,cgr,direction):
    count[0] += 1
    box = k.boxes.xyxy.squeeze().tolist()
    right = k.keypoints.xy[0][direction]
    # print("there")
    length=int(0.3*(box[2]-box[0]))
    width=int(0.3*(box[3]-box[1]))

    box[1] = np.max([int(right[1]) - int(1.2*width),0])
    box[3] = np.min([int(right[1]) + int(0.5*width),img.shape[0]])
    box[0] = np.max([int(right[0]) - length,0])
    box[2] = np.min([int(right[0]) + length,img.shape[1]])
